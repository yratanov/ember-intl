...
function buildTranslationTree(project, inputPath, treeGenerator) {
  ...
  if (fs.existsSync(projectTranslations)) {
    trees.push(treeGenerator(projectTranslations));
  }
...
function processAddons(addons, translationTrees, treeGenerator) {
  addons.forEach(addon => processAddon(addon, translationTrees, treeGenerator));
}

function processAddon(addon, translationTrees, treeGenerator) {
  ...
  if (addon.treeForTranslations) {
    const additionalTranslationTree = addon.treeForTranslations(addonGeneratedTree);
